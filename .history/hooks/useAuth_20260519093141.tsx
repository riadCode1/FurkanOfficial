...
console.log("AuthProvider rendered, session:", CLIENT_ID, DISCOVERY, REDIRECT_URI);
  ...
  return (
    <AuthContext.Provider
      value={{ session, profile, isLoading, error, bookmarks,setBookmarks, login, logout, getAccessToken }}
    >
      {children}
    </AuthContext.Provider>
  );
...
async function exchangeOnBackend(params: {
  ...
  payload.user ??
  (payload.idToken ? jwtDecode<UserProfile>(payload.idToken) : { sub: "unknown" });
