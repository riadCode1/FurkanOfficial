    ...
    <View style={{ flex: 1 }}>
          ...
          <View style={styles.tabsRow}>
            {tabConfig.map((tab, index) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  // Move the indicator smoothly
                  indicatorPosition.value = index * TAB_WIDTH;
                  // Navigate to the tab
                  // Expo Router handles this automatically when using <Tabs.Screen>
                  // But we can force navigation if needed with router
                }}
                activeOpacity={0.8}
              >
                <tab.icon
                  ...
                  size={26}
                  color={Colors.textGray} // You can make this dynamic later
                />
      ...
      {!isKeyboardVisible && track && (
        <BottomBar
          ...
          idReader={idReader}
          chapterId={chapterId}   // ← fixed if you had typo
          arabicCH={arabicCH}
          name={reciter}
        />
      )}
...
const styles = StyleSheet.create({
  ...
  indicator: {
    ...
    bottom: 8,
    height: 5,
    width: TAB_WIDTH * 0.6,        // nice pill width
