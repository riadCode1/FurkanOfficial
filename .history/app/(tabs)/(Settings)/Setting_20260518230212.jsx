...
const Row = ({ icon, iconColor = "#5b8dee", iconBg = "#2a3a6e", label, right, onPress }) => (
  <TouchableRipple disabled={!onPress} onPress={onPress} rippleColor="rgba(200,200,200,0.08)" style={S.row}>
...
// ─── guest banner (replaces profile card only) ────────────────────────────────

const GuestBanner = ({ onLogin, t }) => (
  <TouchableRipple style={S.guestBanner} onPress={onLogin} rippleColor="rgba(91,141,238,0.12)">
    <View style={S.guestBannerInner}>
      <View style={S.guestAvatar}>
        <MaterialIcons name="person-outline" size={22} color={Colors.blue} />
      </View>
      <View style={{ flex: 1 }}>
        <Text style={S.guestName}>{t("غير مسجّل الدخول", "Not logged in")}</Text>
        <Text style={S.guestSub}>{t("اضغط لتسجيل الدخول", "Tap to sign in")}</Text>
      </View>
      <View style={S.loginChip}>
        <MaterialIcons name="login" size={15} color="white" />
        <Text style={S.loginChipTxt}>{t("دخول", "Log In")}</Text>
      </View>
    </View>
  </TouchableRipple>
);
...
const ProfileCard = ({ profile, initial, username, email, onLogout, t }) => (

  console.log(profile.avatarUrls.medium) ||
  <View style={S.profileCard}>
    <TouchableRipple style={S.profileLeft} rippleColor="rgba(200,200,200,0.06)" >
      <View style={S.profileLeftInner}>
        
        <View> 
          <Text style={S.profileName}>{username}</Text>
          {!!email && <Text style={S.profileEmail}>{email}</Text>}
          <Text style={S.profileHint}>{profile?.firstName} { profile?.lastName}</Text>
        </View>
        <View style={S.avatar}><Image contentFit="cover" source={{ uri: " LOG  https://lh3.googleusercontent.com/a/ACg8ocKxX5MbeFhX8CCMySs870_Ua4LtlsLhe3uwiMQXlbkW9k_nEK4=s96-c"}}  style={{width: 40, height: 40, borderRadius: 20}} /></View>
      </View>
...
export default function Setting() {
  ...
  const handleRate = async () => {
    if (await StoreReview.hasAction()) StoreReview.requestReview();
      ...
      : "https://play.google.com/store/apps/details?id=com.riadp1.Quran"
    );
  };
  ...
  return (
    <SafeAreaView style={S.container}>
      ...
      {/* modals — only relevant when logged in */}
      {!isGuest && (
        <>
          <ConfirmModal
      ...
      <ScrollView contentContainerStyle={S.scroll} showsVerticalScrollIndicator={false}>
        ...
        {/* profile OR guest banner */}
        {isGuest
          ? <GuestBanner onLogin={login} t={t} />
          : <ProfileCard profile={profile} initial={initial} username={username} email={email} onLogout={() => setShowLogout(true)} t={t} />
        }
        ...
        <Section label={t("حول التطبيق", "About & Support")}>
          ...
          <Divider />
          <Row icon="mail-outline" iconColor="#4ec9b0" iconBg="#0c2622" label={t("تواصل معنا", "Contact Us")} onPress={() => Linking.openURL("mailto:riadzergui20@gmail.com")} />
          <Divider />
...
const S = StyleSheet.create({
  ...
  // guest banner
  guestBanner:      { marginHorizontal: 16, marginTop: 16, borderRadius: 14, overflow: "hidden", backgroundColor: "rgba(255,255,255,0.05)", borderWidth: 1, borderColor: "rgba(91,141,238,0.2)" },
  guestBannerInner: { flexDirection: "row", alignItems: "center", padding: 12, gap: 12 },
  guestAvatar:      { width: 38, height: 38, borderRadius: 19, backgroundColor: "rgba(91,141,238,0.12)", borderWidth: 1, borderColor: "rgba(91,141,238,0.25)", justifyContent: "center", alignItems: "center" },
  guestName:        { color: "white", fontSize: 14, fontWeight: "600" },
  guestSub:         { color: "rgba(255,255,255,0.4)", fontSize: 11, marginTop: 2 },
  loginChip:        { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: Colors.blue, paddingHorizontal: 12, paddingVertical: 7, borderRadius: 20 },
  loginChipTxt:     { color: "white", fontSize: 13, fontWeight: "700" },
  ...
  logoutIcon:       { paddingHorizontal: 14, paddingVertical: 10, justifyContent: "center" },
