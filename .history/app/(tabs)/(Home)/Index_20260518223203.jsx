...
// ─── bookmark pill ────────────────────────────────────────────────────────────
const BookmarkPill = ({ bookmark, onDelete }) => (
  <View style={S.bookmarkPill}>
    <MaterialIcons name="bookmark" size={14} color={Colors.blue} />
    <Text style={S.bookmarkKey}>{bookmark.verse_key ?? bookmark.key}</Text>
...
const Index = () => {
  ...
  useEffect(() => {
    fetchBookmarks();
  }, [session]); // re-fetch when session changes (login/logout)
  ...
  return (
    <SafeAreaView style={S.container}>
      ...
      >
        ...
        {/* bookmarks — only if logged in and has bookmarks */}
        {session && (
          <View style={S.section}>
            <SectionHeader title={languages ? "العلامات المرجعية" : "Bookmarks"} />
            {bookmarksLoading ? (
              ...
              <ScrollView
                ...
                contentContainerStyle={S.bookmarkRow}
              >
                {bookmarks.map((b) => (
                  <BookmarkPill key={b.id} bookmark={b} onDelete={removeBookmark} />
                ))}
              </ScrollView>
            ) : (
              <Text style={S.emptyBookmarks}>
                {languages ? "لا توجد علامات مرجعية بعد" : "No bookmarks yet"}
              </Text>
            )}
          </View>
        )}
