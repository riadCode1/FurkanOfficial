...
const PlayList = () => {
  ...
  const hideAlert = () => setAlertVisible(false);
   ...
   const Remove = useCallback(async (item) => {
    // Use functional updater so we always have fresh state — no stale closure
    setPlaylist((prev) => {
      const updated = prev.filter(
        (p) => !(p?.id === item?.id && p?.reciterID === item?.reciterID)
      );
      // Persist to AsyncStorage in background
      AsyncStorage.setItem("playList", JSON.stringify(updated)).catch((e) =>
        console.error("AsyncStorage remove error:", e)
      );
      return updated; // update UI instantly
    });
  ...
  const fetchBookmarks = useCallback(async () => {
    ...
    console.log("Bookmarks updated:", bookmarks); // DEBUG: log bookmarks changes
    ...
    <View style={{ alignItems: "center" }}>
      ...
      {bookmarks[0] ? (
        <FlatList
          ...
          keyExtractor={(item) => item?.reciterID?.toString()}
