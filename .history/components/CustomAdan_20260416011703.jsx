  ...
  const fetchPrayerTimes = async () => {
    try {
      ...
      const response = await axios.get("https://api.aladhan.com/v1/timingsByAddress", {
        params: {
          address: currentLocation? `${currentLocation?.city},${currentLocation?.country}`:"",
      ...
      if (!next) {
        ...
        next = {
          name: firstName,
          time: new Date(`${tomorrow.toISOString().split("T")[0]}T${firstTime}:00`),
        };
      }
...
if(!currentLocation){
   return( <TouchableOpacity onPress={()=>prepare(setCurrentLocation)} style={styles.center}>

    <vector.MaterialIcons color={Colors.blue} size={30} name="add-location"/>
  ...
  return (
    <View style={styles.container}>
      <ImageBackground source={require("../assets/images/isha.png")} style={{borderRadius: 50, overflow: "hidden", width:"100%",justifyContent:"center",alignItems:"center",gap:20}}>
      ...
      {nextPrayer && (
        <View style={{ alignItems: "center", marginBottom: 12 }}>
          <Text style={styles.nextTitle}>
            {languages
              ? `${prayerNames[nextPrayer.name] || nextPrayer.name} بعد`
              : `${nextPrayer.name} after`}
          </Text>
          ...
          {Object.entries(data)
            ...
            .map(([name, time]) => (
              ...
              >
                ...
                {name === "Asr" && (
                  <vector.Ionicons size={20} color="white" name="partly-sunny" />
                )}
                {name === "Dhuhr" && (
                ...
                {name === "Maghrib" && (
                  <vector.FontAwesome5 size={20} color="white" name="cloud-moon" />
                )}
                {name === "Isha" && (
                  <vector.FontAwesome6 size={20} color="white" name="moon" />
                )}
                {name === "Fajr" && (
                  <vector.MaterialIcons size={20} color="white" name="sunny-snowing" />
                )}
...
const styles = StyleSheet.create({
  ...
  nextTitle: { alignSelf: "flex-start", fontSize: 18, fontWeight: "600", color: "white" },
  countdown: {fontWeight:500, fontSize: 35, color: Colors.blue },
});
