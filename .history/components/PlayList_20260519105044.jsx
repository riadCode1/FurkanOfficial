...
const PlayList = () => {
  ...
  // ── local playlist from AsyncStorage ──────────────────────────────────────
  // FIX: `playlist` removed from deps — was causing the infinite loop
  const fetchPlaylist = useCallback(async () => {
    try {
      const raw = await AsyncStorage.getItem("playList");
      const res = JSON.parse(raw);
      setPlaylist(Array.isArray(res) ? res : []);
    } catch (e) {
  ...
  // FIX: only `isFocused` as dep, NOT `playlist`
  useEffect(() => {
    if (isFocused) fetchPlaylist();
  }, [isFocused]);

  // ── API bookmarks (separate from playlist) ────────────────────────────────
  ...
  const Remove = useCallback(async (item) => {
    try {
      const updated = playlist.filter(
        (p) => !(p?.id === item?.id && p?.reciterID === item?.reciterID)
      );
      await AsyncStorage.setItem("playList", JSON.stringify(updated));
      setPlaylist(updated);
      setAlertVisible(true);
      setTimeout(() => setAlertVisible(false), 2500);
      // FIX: original had finally{setAlertVisible(false)} which instantly hid the toast
    } catch (e) {
      console.error("Error removing item:", e);
    }
  }, [playlist]);

  // ── search — derived value, no extra state/effect needed ──────────────────
  // FIX: was a useState+useEffect combo that was always one render behind
  const filteredData = searchQuery.length > 1
    ...
    : playlist; // always show full playlist when no search
  ...
  return (
    <View style={S.root}>
      ...
      {/* FIX: was checking bookmarks.length but rendering playlist — now consistent */}
      {filteredData.length === 0 ? (
        ...
        <FlatList
          ...
          // FIX: id alone isn't unique across reciters
          keyExtractor={(item, index) => `${item?.reciterID}-${item?.id}-${index}`}
